...
} from "~/types/dashboardBookings.types";
...
function sortItems(
  ...
  switch (sort) {
    ...
    case "price_low":
      return arr.sort(
        (a, b) => Number.parseFloat(a.total) - Number.parseFloat(b.total),
      );
    case "newest":
    default:
      return arr.sort(
...
export function useDashboardBookings(initialTab: BookingTab = "pool") {
  ...
  const allLoaded = computed(
    () => currentPage.value > 0 && currentPage.value >= totalPages.value,
  );
  ...
  const needsFullData = computed(
    () => !!search.value.trim() || sort.value !== "newest",
  );

  async function fetchOnePage(page: number, gen: number): Promise<boolean> {
    const result = await safeApiFetch<DashboardBookingsResponse>(
    ...
    rawItems.value = rawItems.value.map((item) =>
      item.id === id
        ? {
            ...item,
            status: "cancelled",
            status_text: BOOKING_STATUS_LABELS.cancelled,
          }
        : item,
    );
    return true;
  ...
  const items = computed(() => {
    ...
    return sortItems(
      rawItems.value.filter((item) => matchesSearch(item, search.value)),
      sort.value,
    );
  });
  ...
  const isEmpty = computed(
    () => !isBusy.value && !error.value && items.value.length === 0,
  );
